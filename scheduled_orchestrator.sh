...
ORCHESTRATOR_SCRIPT="/home/gprice/projects/multiverse_dive/orchestrator.sh"
...
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}   Scheduled Orchestrator - Autonomous Build System${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}\n"
...
# Initialize log file
echo "==================================" > "$LOGFILE"
echo "Scheduled Orchestrator Run Log" >> "$LOGFILE"
...
while true; do
    ...
    echo -e "\n${BLUE}═══════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}[Run ${RUN_COUNT}/${MAX_RUNS}]${NC} $(date)"
    ...
    if "$ORCHESTRATOR_SCRIPT" >> "$LOGFILE" 2>&1; then
        echo -e "${GREEN}✓ Orchestrator run #${RUN_COUNT} completed successfully${NC}"
        echo "Status: SUCCESS" >> "$LOGFILE"
    else
        EXIT_CODE=$?
        echo -e "${RED}✗ Orchestrator run #${RUN_COUNT} failed with exit code ${EXIT_CODE}${NC}"
