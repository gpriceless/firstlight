...
- **Query API:** New `/control/v1/context/` endpoints let the partner platform query accumulated context by geometry and type
...
- **SSE events for lakehouse:** The lakehouse grows silently. The partner platform does not receive streamed events when context rows are inserted. Lakehouse data is queryable on demand.
...
- Partner platform can query `/control/v1/context/buildings?bbox=...` and receive accumulated building footprints
